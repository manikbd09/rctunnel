...
yum --enablerepo=epel install openvpn stunnel chkconfig initscripts jq mysql redhat-lsb -y
...
mysql -u $USER -p$PASS -D $DB -h $HOST -e "UPDATE server_list SET online='$total' WHERE server_ip='$server_ip' "
LENZ05
...
Contact(Whatsapp): +639703855290... " >> /usr/.web/index.php
}
}
...
echo 'Starting..'
{
sudo crontab -l | { echo "* * * * * pgrep -x stunnel >/dev/null && echo 'GOOD' || /etc/init.d/stunnel restart"; } | crontab -
} 
